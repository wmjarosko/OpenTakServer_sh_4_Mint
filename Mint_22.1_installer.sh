...
if [ "$NAME" != "Linux Mint" ]
then
  read -p "${YELLOW} This installer is for Linux Mint but this system is $NAME. Do you want to run anyway? [y/N] ${NC}" confirm < /dev/tty && [[ $confirm == [yY] || $confirm == [yY][eE][sS] ]] || exit 1
  rm -fr $INSTALLER_DIR
fi
...
python3 -m venv --system-site-packages ~/.opentakserver_venv --break-system-packages
source "$HOME"/.opentakserver_venv/bin/activate
python3 -m pip install --upgrade pip setuptools wheel --break-system-packages
pip3 install opentakserver --break-system-packages
...
# On Mint 25.04 and up the PLUGINS_DIR variable needs to be set in order to enable plugins
IFS=" "
